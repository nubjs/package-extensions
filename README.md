...
**779 package names.** The scan covers 9,982 of the top 10,000 and finds 648 with an undeclared dependency across 1,167 edges. The Yarn seed contributes 142 package names, including 131 not found by the scan.
...
| --- | --- | --- | --- |
| `types` | 533 | Only a `.d.ts` references it. No runtime edge at all. | optional peer |
| `guarded` | 299 | Every occurrence sits inside a try/catch or a conditional branch. | optional peer |
| `runtime` | 204 | The main entry graph imports it, unguarded. | optional peer, or `dependencies` once reviewed |
| `adapter` | 131 | A non-`.` exports subpath imports a backend the consumer chose. | optional peer |
...
- **A whole tier is withheld.** Node's legacy resolution lets a consumer import any published file by path, and seeding those files finds real entry points — `redux-persist/lib/integration/react` is why the pass exists. It also finds template payloads a generator copies out, browser bundles served over HTTP, and specifiers only a bundler resolves. Those 246 edges are recorded under `withheldDeepPath` and emitted nowhere; [`docs/deep-path-tier.md`](docs/deep-path-tier.md) walks the seven classes and shows why a filename rule does not separate them.
